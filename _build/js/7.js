(window["AnthonysContracting"] = window["AnthonysContracting"] || []).push([[7],{
...
var Contact = function (_Module) {
    _inherits(Contact, _Module);

    function Contact(el, opts) {
        _classCallCheck(this, Contact);

        return _possibleConstructorReturn(this, (Contact.__proto__ || Object.getPrototypeOf(Contact)).call(this, el, opts));
    }
    ...
    _createClass(Contact, [{
        ...
        value: function onFormSubmit(e) {
            ...
            var validated = true;
            var fields = [{ val: $.trim(this.dom.formName.val()), pattern: null }, { val: $.trim(this.dom.formPhone.val()), pattern: _regex.phoneRegexPattern }, { val: $.trim(this.dom.formEmail.val()), pattern: _regex.emailRegexPattern }, { val: $.trim(this.dom.formFound.val()), pattern: null }, { val: $.trim(this.dom.formMsg.val()), pattern: null }].forEach(function (item, idx) {
                validated = !(item.val === '') && !(item.pattern !== null && !item.pattern.test(item.val));
                _gsap.TweenMax.to(_this3.dom.required.eq(idx).toggleClass('show', !validated), 0.5, { autoAlpha: !validated, height: !validated ? 35 : 0, ease: _gsap.Power4.easeOut });
            });
            if (validated) {
        ...
        value: function postContactUsData(fields) {
        ...
        key: 'onPostContactUsData',
        value: function onPostContactUsData(fields) {
            this.postContactUsData(fields).then(function () {
